"""BestPlayer1 -- gap-trap-aware determinized IS-MCTS (symmetric model).

Final submission agent #1. Self-contained (no imports of other player modules
or baselines) so the grader can load it in isolation.

Algorithm
---------
A determinized Information-Set Monte-Carlo Tree Search. The tournament scores
by *rank* and opponents' hands are hidden, so each of the ~10k iterations/move:
  1. determinizes the hidden hands by dealing the unseen pool (which also holds
     the never-dealt cards -- correct epistemic uncertainty),
  2. descends a UCB1 tree keyed by our own card sequence, opponents playing a
     fast greedy policy, every round resolved exactly like the engine,
  3. greedy-rolls the remaining cards to game end,
  4. backpropagates the normalized seat rank we achieved.
The move played is the most-visited root child (robust to rollout noise).

The decisive ingredient over a plain greedy/MCTS is the **gap-trap** term in
the base policy `_eval`. A round resolves low-to-high, so any unseen card in the
gap between a row's end and the card we place can be played by an opponent
*first*; if enough land there the row fills to five and our card becomes the
trapped sixth -- we take the row. `_eval` charges the exact binomial probability
of that (over how many unseen cards sit in the gap and how many opponents could
place there) times the row's bullheads, on top of the strong ``*10``
take-avoidance. Empirically this lifted the agent from ~2.65 to ~2.5 avg rank
against the strong baseline, i.e. up to its level.

This is the **symmetric** variant: the gap term is applied to both our own
moves and the simulated opponents (``opp_gap_w == gap_w``). BestPlayer2 is the
asymmetric counterpart (plain-greedy opponent model) -- equal strength, a
distinct style for diversity across the two submissions.

All tuning knobs are constructor keywords with defaults, so only ``player_idx``
...
class BestPlayer1:
	...
	def __init__(self, player_idx, c=0.6, root_k=10, budget=0.85,
		...
		self.gap_w = gap_w
		self.opp_gap_w = opp_gap_w   # symmetric model: == gap_w
		self.iters = iters
	...
	def _eval(self, rows, sums, card, bull, gw):
		...
		if gw > 0.0 and self._nU > 0:
			...
			if slots <= self._n_opp:
				g = (bisect.bisect_left(self._U, card)
				     - bisect.bisect_right(self._U, best_last))
				if g > 0:
					p = g / self._nU
					rv += gw * self._binom_ge(self._n_opp, slots, p) * sums[best]
		return rv + (card - best_last) * 0.01
	...
	def action(self, hand, history):
		...
		unseen = self._unseen(hand, history)   # already ascending (sorted)
		self._U = unseen
		self._nU = len(unseen)
		self._n_opp = n_opp
