"""BestPlayer2 -- gap-trap-aware determinized IS-MCTS (asymmetric model).

Final submission agent #2. Self-contained (no imports of other player modules
or baselines) so the grader can load it in isolation.

Identical search and gap-trap policy to BestPlayer1 (see that file's docstring
for the full algorithm description) with one deliberate difference: the
**opponent model is asymmetric**. We avoid the gap trap aggressively for our
own moves (``gap_w=1.0``) but model the simulated opponents with the plain
greedy policy (``opp_gap_w=0.0``), on the theory that real opponents do not
dodge the gap trap the way we do, so charging them our own caution biases the
rollouts. In testing this is statistically equal in strength to the symmetric
BestPlayer1 but plays a measurably different game -- the point is *diversity*:
two strong agents with distinct behaviour so the pair is robust to whatever
opponent pool the tournament draws.
...
class BestPlayer2:
	...
	def __init__(self, player_idx, c=0.6, root_k=10, budget=0.85,
		...
		self.gap_w = gap_w
		self.opp_gap_w = opp_gap_w   # asymmetric model: plain-greedy opponents
		self.iters = iters
