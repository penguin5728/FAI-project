...
class player2:
	"""Depth-1 determinization Monte Carlo agent for 6 Nimmt! (robust rank).

	Shares player1's sum-cached depth-1 determinization machinery -- deal the
	unseen pool into plausible opponent hands, play each candidate first move
	out to the end with a fast greedy policy for everyone, and score by the
	rank it yields for us. The difference is the objective: instead of the
	plain mean rank, player2 minimizes a *risk-averse* score,
	``mean_rank + 0.5 * std_rank``. This penalizes candidates whose outcome
	swings wildly across determinizations and prefers moves that are reliably
	decent against an unknown opponent pool -- a genuinely distinct style from
	player1's expected-rank optimization.

	The board carries per-row bullhead sums incrementally (never re-summed),
	so within the time budget we run many determinizations; the resulting
	variance reduction is what makes both the mean and the spread estimates
	trustworthy. All candidates share the same deal each iteration (common
	random numbers). A per-rollout hard cap keeps us under the 1.0s timeout.
	"""
	...
	def __init__(self, player_idx):
		...
		self.rng = random.Random(20260614 + player_idx * 2654435761)
	...
	def action(self, hand, history):
		...
		for c in candidates:
			...
			if var < 0.0:
				var = 0.0
			key = (mr + self.RISK * var ** 0.5, pen_tot[c] / n, -c)
